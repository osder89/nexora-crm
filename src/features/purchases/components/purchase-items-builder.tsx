...
export function PurchaseItemsBuilder({ products }: PurchaseItemsBuilderProps) {
  const [rows, setRows] = useState<ItemRow[]>([
    { key: crypto.randomUUID(), productId: "", quantityInput: "1", unitCostInput: "1" },
  ]);

  const totalPreview = useMemo(
    () =>
      rows.reduce((sum, row) => {
        const quantity = parseOptionalIntInput(row.quantityInput);
        const unitCost = parseOptionalNumberInput(row.unitCostInput);
  ...
  function addRow() {
    setRows((current) => [...current, { key: crypto.randomUUID(), productId: "", quantityInput: "1", unitCostInput: "1" }]);
  }
        ...
        <div key={row.key} className="grid gap-2 rounded-md border border-slate-200 p-3 md:grid-cols-12">
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              onChange={(event) => updateRow(row.key, { quantityInput: event.target.value })}
              className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
            />
          </div>
          ...
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(event) => updateRow(row.key, { unitCostInput: event.target.value })}
              className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
            />
          </div>
