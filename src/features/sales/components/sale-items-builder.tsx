...
import { parseOptionalIntInput } from "@/shared/lib/numeric-input";
...
export function SaleItemsBuilder({ products, onTotalChange }: SaleItemsBuilderProps) {
  const [rows, setRows] = useState<ItemRow[]>([{ key: crypto.randomUUID(), productId: "", quantityInput: "1" }]);
  ...
  function addRow() {
    setRows((current) => [...current, { key: crypto.randomUUID(), productId: "", quantityInput: "1" }]);
  }
