...
import { CollectPaymentModalButton } from "@/features/receivables/components/collect-payment-modal-button";
import type { ReceivableSale } from "@/services/server/receivables.service";
...
import { formatCurrency, formatDate, startOfDay } from "@/shared/lib/utils";

type ReceivablesSegment = "all" | "pending" | "upcoming" | "overdue";

type ReceivablesPageProps = {
  q: string;
  segment: ReceivablesSegment;
  sellerId: string;
  page: number;
...
export default function ReceivablesPage({ q, segment, sellerId, page, sales: receivableSales, users }: ReceivablesPageProps) {
  ...
  const filteredSales = receivableSales.filter((sale) => {
    const customerLabel = sale.customer ? getCustomerFullName(sale.customer) : "Consumidor final";
    const haystack = [sale.id, customerLabel, sale.customer?.phone ?? "", sale.customer?.nit ?? "", sale.seller?.email ?? ""].join(" ").toLowerCase();
    const matchesSeller = sellerId ? sale.seller?.id === sellerId : true;
    const matchesQuery = !q || haystack.includes(q);

    return matchesSeller && matchesQuery;
  });

  const dueData = filteredSales
    .map((sale) => {
      const nextInstallment = sale.installments.find((installment) => installment.status === InstallmentStatus.PENDING || installment.status === InstallmentStatus.OVERDUE) ?? null;
      const nextDueDate = nextInstallment?.dueDate ? new Date(nextInstallment.dueDate) : sale.dueDate ? new Date(sale.dueDate) : null;
...
function buildReceivablesUrl({ q, segment, sellerId, page }: { q: string; segment: ReceivablesSegment; sellerId: string; page: number }) {
