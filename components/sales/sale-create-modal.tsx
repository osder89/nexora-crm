...
import { SaleCreateForm, SaleProductOption } from "@/components/sales/sale-create-form";
...
export function SaleCreateModalButton({ customers, products }: SaleCreateModalButtonProps) {
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4">
          <div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            ...
            <SaleCreateForm customers={customers} products={products} onSuccess={() => setOpen(false)} />
          </div>
        </div>
      ) : null}
    </>
